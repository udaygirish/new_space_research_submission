...
def main():
    ...
    parser.add_argument(
        ...
        "--model_path",
        required=True,
        help="saved model path (folder name) or frozen graph file path",
    )
    parser.add_argument("-i", "--image_path", required=True, help="Image path")
    parser.add_argument(
        "-o", "--output_path", default="output", help="output image path"
    )
    ...
    parser.add_argument(
        ...
        "--iou_calc",
        default=True,
        help="To specify whether to calculate mAP or not",
    )
    ...
    for i, img_path in tqdm(enumerate(file_paths)):
        ...
        if args.iou_calc == True:
            avg_iou, avg_w_iou = compute_avg_iou(img_path, detections, iou_threshold)
        else:
            pass
