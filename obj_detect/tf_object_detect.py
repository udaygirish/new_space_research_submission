...
class ObjectDetection(object):
    def __init__(self, saved_model_dir_or_frozen_graph):
        ...
        with open("config/label_map.json") as json_file:
            self.idx2label = json.load(
                json_file, object_hook=lambda d: {int(k): v for k, v in d.items()}
            )
        ...
        with open("config/config.json") as json_file:
            ...
            self.threshold = data.get("min_conf_threshold", 0.0)
    ...
    def predict(self, image_arrays):
        ...
        if self.model_type == "tf2_od":
            ...
            filtered_detections = self.format_tf_od_predictions(
                predictions, img_h, img_w, self.threshold, self.filter_classes
            )
            ...
            filtered_detections = self.format_effnet_predictions(
                predictions[0], self.threshold, self.filter_classes
            )
        else:
            predictions = self.sess.run(
                self.signatures["prediction"],
    ...
    def format_tf_od_predictions(
        self, predictions, img_h, img_w, threshold, filter_classes=[]
    ):
        ...
        for i in range(int(num_detections)):
            ...
            if score >= threshold:
                if (len(filter_classes) >= 1 and obj_class in filter_classes) or len(
                    filter_classes
                ) == 0:
                    y1, x1, y2, x2 = boxes[i]
                    y1_o = y1 * img_h
