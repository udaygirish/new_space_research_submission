...
class IOU_PR_Calc:
    ...
    def associate_keys_calc_iou(self, dict1, dict2):
        ...
        for i in dict1.keys():
            ...
            for j in dict2.keys():
                temp_euc_dist = round(
                    self.find_euclidean_distance(dict1[i][0], dict2[j][0]), 2
                )
    ...
    def _compute_iou(self, image_path, detections):
        ...
        self.ass_keys, self.ass_iou = self.associate_keys_calc_iou(
            self.bbox_pc_dict, self.bbox_gc_dict
        )
...
def compute_avg_iou(image_path, detections, threshold):
    ...
    ass_iou, ass_keys, l_gc_coord, l_pc_coord = iou_calc._compute_iou(
